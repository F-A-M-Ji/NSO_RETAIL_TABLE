...
def initialize_calculate_dataframe(df_bign_input):
    ...
    expected_unique_tsic = 56  # FoxPro: FOR i = 1 TO 56 for 4-digit
...
def perform_main_calculations(conn, df_calculate_temp, df_bign_input):
    ...
    for index, row in df_calculate_temp.iterrows():
        ...
        for i in range(1, 13):
            ...
            if wi != 9999.9999:
                df_calculate_temp.loc[index, f"w{i}"] = round(
                    wi, 4
                )  # ปัดเศษ w เป็น 4 ตำแหน่ง
            else:
...
def perform_validation_and_update_report(conn, df_calculate_temp):
    ...
    output_csv_filename = (
        f"calculate_{TSIC_LENGTH}digit_YR{yr_val}_QTR{qtr_val}.csv"
    )
...
def run_processing_pipeline_4digit(conn):
    yr = PARAMS_CONFIG["YR"]
